...
def train_distilbert(df_train, df_test, df_validate, text_col='text', label_col='label', epochs=3, batch_size=16, lr=2e-5):
    tokenizer = DistilBertTokenizer.from_pretrained('distilbert-base-uncased')
    model = DistilBertForSequenceClassification.from_pretrained('distilbert-base-uncased', num_labels=len(np.unique(df_train[label_col].values, return_counts=False)))
