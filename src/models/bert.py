...
def train_bert(df_train, df_test, df_validate, text_col='text', label_col='label', epochs=3, batch_size=16, lr=2e-5):
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
    model = BertForSequenceClassification.from_pretrained('bert-base-uncased', num_labels=len(np.unique(df_train[label_col].values, return_counts=False)))
